    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        if (confidence < 0.9) {
          ...
          const { scanId, taskId, result } = await ingestFormForProcessing(
            base64, 
          ...
          if (taskId.startsWith('local-')) {
            ...
            console.log(`[INGESTION] Form completed immediately on backend: ${scanId}`);
            setPendingResults({ url, data: { ...result, status: 'completed' } });
          } else {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 relative">
        {currentPages.map((page, i) => (
          <div 
            ...
            className="relative aspect-[3/4] bg-surface-container-low rounded-3xl overflow-hidden border border-outline-variant/10 shadow-sm group cursor-pointer hover:ring-4 hover:ring-primary/20 transition-all"
          >
            <img src={page.url} className="w-full h-full object-cover" alt={`Page ${i+1}`} />
            
            {/* Neural Pulse for AI Results */}
            {page.data.extractionTier === 'AI_SMART' && (
              <div className="absolute inset-x-0 bottom-0 h-1 bg-primary/30 overflow-hidden">
                <motion.div 
                  animate={{ x: ['-100%', '100%'] }}
                  transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
                  className="w-1/2 h-full bg-primary shadow-[0_0_10px_#3b82f6]"
                />
              </div>
            )}
            ...
            <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-3">
                ...
                <span className="text-white font-black text-[10px] uppercase tracking-[0.2em] bg-white/10 backdrop-blur-md px-4 py-1.5 rounded-full border border-white/20">
                  {page.data.extractionTier === 'AI_SMART' ? 'Open Authority Review' : 'View Extraction'}
                </span>
            ...
            {/* Badge */}
            <div className={`absolute top-4 left-4 px-3 py-1 rounded-full text-[8px] font-black tracking-widest uppercase flex items-center gap-1.5 backdrop-blur-md border ${page.data.extractionTier === 'AI_SMART' ? 'bg-primary/20 text-primary border-primary/30' : 'bg-surface/50 text-on-surface-variant border-outline-variant/30'}`}>
              <div className={`w-1.5 h-1.5 rounded-full ${page.data.extractionTier === 'AI_SMART' ? 'bg-primary animate-pulse' : 'bg-on-surface-variant'}`} />
              {page.data.extractionTier === 'AI_SMART' ? 'Hydra AI' : 'Deterministic'}
            </div>
          </div>
        ))}
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-3 mb-1">
            <div className="flex items-center gap-1.5 px-3 py-1 bg-surface-container-highest rounded-full border border-primary/20 shadow-lg shadow-primary/5">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.1em] text-primary">Hydra V10.1 Engine Active</span>
            </div>
